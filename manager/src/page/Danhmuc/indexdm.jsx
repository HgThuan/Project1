...
export default function Indexdm() {
    ...
    const deleteDM = (ma_danh_muc) => {
        if (window.confirm("Bạn có muốn xóa danh mục này không ?")) {
            axios.delete(`http://localhost:5001/api/deletedm/${ma_danh_muc}`);
                                ...
                                {data.map((item, index) => {
                                    return (
                                        ...
                                        <tr key={item.ma_danh_muc}>
                                            ...
                                            <td><Link to={`/Viewdm/${item.ma_danh_muc}`} type="button" className="btn btn-primary">Chi Tiết</Link></td>
                                            <td><Link to={`/Updatedm/${item.ma_danh_muc}`} className="btn btn-warning">Sửa</Link></td>
